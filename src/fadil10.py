...
# PERHATIAN: Nilai-nilai di config.py sekarang harus dalam format HSL (H_low, S_low, L_low, H_high, S_high, L_high)
white_trackbar = TrackbarManager("white_trackbar")
# Asumsi WHITE_SETTING di config.py akan diupdate ke format HSL
white_trackbar.add_multiple(config.WHITE_SETTING)

yellow_trackbar = TrackbarManager("yellow_trackbar")
# Asumsi YELLOW_SETTING di config.py akan diupdate ke format HSL
...
try:
    while(True):
        ...
        # --- 2. Segmentasi Warna (Masking) pada BEV Frame - Konversi ke HSL ---
        warped_hsl = cv2.cvtColor(warped_frame, cv2.COLOR_BGR2HLS) # Konversi ke HSL

        # Segmentasi Warna Pertama (Putih)
        # Format trackbar sekarang: H_low, S_low, L_low, H_high, S_high, L_high
        white_hsl_value = white_trackbar.get_values_list()
        lower_white = np.array([white_hsl_value[0], white_hsl_value[1], white_hsl_value[2]])
        upper_white = np.array([white_hsl_value[3], white_hsl_value[4], white_hsl_value[5]])
        mask_white = cv2.inRange(warped_hsl, lower_white, upper_white)

        # Segmentasi Warna Kedua (Kuning)
        yellow_hsl_value = yellow_trackbar.get_values_list()
        lower_yellow = np.array([yellow_hsl_value[0], yellow_hsl_value[1], yellow_hsl_value[2]])
        upper_yellow = np.array([yellow_hsl_value[3], yellow_hsl_value[4], yellow_hsl_value[5]])
        mask_yellow = cv2.inRange(warped_hsl, lower_yellow, upper_yellow)
        ...
        cv2.imshow('Bird Eye View (HSL)', warped_hsl) # Tambahan tampilan HSL
        cv2.imshow('Binary Mask (from BEV) - Combined', binary_warped)
        cv2.imshow('Mask White', mask_white)
        cv2.imshow('Mask Yellow', mask_yellow)
        # cv2.imshow('Mask Additional Color', mask_additional) # Dihapus
