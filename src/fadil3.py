...
import config # Pastikan config.py memiliki WHITE_SETTING dan YELLOW_SETTING
...
# Mengaktifkan kembali yellow_trackbar untuk segmentasi kedua
yellow_trackbar = TrackbarManager("yellow_trackbar")
yellow_trackbar.add_multiple(config.YELLOW_SETTING)

# trackbar_dirt (jika masih ada di config.py dan tidak ingin dihapus)
...
try:
    while(True):
        ...
        # --- 2. Segmentasi Warna (Masking) pada BEV Frame - DUA WARNA DIGABUNGKAN ---
        ...
        # Piksel yang cocok dengan putih ATAU kuning akan dipertahankan
        binary_warped = cv2.bitwise_or(mask_white, mask_yellow)
        # --- Akhir Segmentasi Warna ---


        # --- 3. Analisis Histogram ---
        # Histogram sekarang bekerja pada binary_warped yang sudah digabungkan
