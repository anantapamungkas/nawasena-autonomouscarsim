...
# Trackbar lain yang tidak digunakan dalam deteksi warna tunggal bisa dipertahankan
dirt_trackbar = TrackbarManager("dirt_trackbar")
dirt_trackbar.add_multiple(config.DIRT_SETTING)

yellow_trackbar = TrackbarManager("yellow_trackbar") # Trackbar ini tetap ada, tapi tidak digunakan untuk masking
...
# Anda mungkin perlu menyesuaikan nilai-nilai ini berdasarkan simulator Anda.
# Contoh:
# Asumsi 1 lane width = 3.7 meter
# Asumsi BEV_WIDTH (640px) mencakup sekitar 2 lane (7.4 meter)
# Asumsi BEV_HEIGHT (480px) mencakup sekitar 30 meter ke depan
...
try:
    while(True):
        ...
        if left_fit is not None and right_fit is not None:
            ...
            # Polinomial jalur tengah
            center_fit = (left_fit + right_fit) / 2

            # Konversi koefisien polinomial ke meter
            # y_eval_m = ploty[-1] * YM_PER_PIX # Posisi y di bagian bawah frame BEV dalam meter
            # Untuk kelengkungan, kita sering mengevaluasi di titik terdekat dengan mobil (bottom of image)
            y_eval_pixels = BEV_HEIGHT - 1 # y di bagian bawah gambar BEV (piksel)

            # Koefisien A dan B dalam satuan meter
            # x = A_m * y_m^2 + B_m * y_m + C_m
            # x_pixels = (A_m / XM_PER_PIX) * (y_pixels * YM_PER_PIX)^2 + (B_m / XM_PER_PIX) * (y_pixels * YM_PER_PIX) + (C_m / XM_PER_PIX)
            # x_pixels = (A_m * YM_PER_PIX^2 / XM_PER_PIX) * y_pixels^2 + (B_m * YM_PER_PIX / XM_PER_PIX) * y_pixels + (C_m / XM_PER_PIX)
            # Jadi, A_pixels = A_m * YM_PER_PIX^2 / XM_PER_PIX
            # A_m = A_pixels * XM_PER_PIX / YM_PER_PIX^2
            A_m = center_fit[0] * XM_PER_PIX / (YM_PER_PIX**2)
            B_m = center_fit[1] * XM_PER_PIX / YM_PER_PIX
            # C_m = center_fit[2] * XM_PER_PIX # Ini adalah offset awal dalam meter

            # Turunan pertama dan kedua untuk kelengkungan
            first_deriv = 2 * A_m * y_eval_pixels * YM_PER_PIX + B_m # dx/dy dalam meter
            second_deriv = 2 * A_m # d^2x/dy^2 dalam meter

            # Radius Kelengkungan (dalam meter)
            # Menambahkan penanganan jika second_deriv sangat dekat dengan nol
            if np.abs(second_deriv) < 1e-6: # Hindari pembagian dengan nol untuk jalur lurus
                curvature = 10000.0 # Angka besar untuk kelengkungan sangat kecil (mendekati lurus)
            else:
                curvature = ((1 + first_deriv**2)**1.5) / np.abs(second_deriv)

            # Offset Jalur (dalam meter)
            # Posisi x pusat mobil di BEV (asumsi di tengah bawah)
            car_center_x_pixels = BEV_WIDTH / 2
            # Posisi x pusat jalur di bagian bawah BEV
            lane_center_bottom_x_pixels = center_fit[0]*y_eval_pixels**2 + center_fit[1]*y_eval_pixels + center_fit[2]
            
            # Offset dari pusat mobil ke pusat jalur (positif jika jalur di kanan mobil, negatif jika di kiri)
            ...
            # Menggunakan rumus yang Anda berikan
            # Pastikan curvature tidak nol untuk menghindari error
            if curvature == 0: # Jika jalur lurus sempurna, anggap kelengkungan sangat besar
                steering_angle = np.arctan(lane_offset / 10000.0) * 180 / np.pi # Hindari div by zero
            else:
                steering_angle = np.arctan(lane_offset / curvature) * 180 / np.pi

            # --- Visualisasi Garis Kemudi di Frame Asli ---
            # Asumsi titik awal garis di tengah bawah frame asli
            line_start_x = frame.shape[1] // 2 # 320
            line_start_y = frame.shape[0]     # 640

            line_length = 150 # Panjang garis yang akan digambar (piksel)
            # Menghitung titik akhir berdasarkan sudut kemudi
            # Sudut 0 adalah lurus ke depan (y negatif), positif ke kanan, negatif ke kiri
            end_x = int(line_start_x + line_length * np.sin(np.radians(steering_angle)))
            end_y = int(line_start_y - line_length * np.cos(np.radians(steering_angle))) # Y berkurang saat naik

            cv2.line(result_original, (line_start_x, line_start_y), (end_x, end_y), (255, 0, 0), 3) # Garis biru tebal
            ...
            # Jika fit polinomial gagal, tampilkan frame asli tanpa overlay jalur/metrik
            cv2.imshow('Lanes on Original Frame', result_original)
            cv2.imshow('Lanes on Bird Eye View', out_img) # Tampilkan jendela sliding window jika fit gagal
            print("No valid lane lines detected for fitting.")

        # --- Tampilan Hasil Debugging ---
        cv2.imshow('Original Frame (640x640)', frame) # Ini frame asli dengan titik kalibrasi
