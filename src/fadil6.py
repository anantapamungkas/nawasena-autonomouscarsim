...
import config # Pastikan config.py memiliki WHITE_SETTING, YELLOW_SETTING, dan setelan ketiga dalam format RGB
...
additional_color_trackbar = TrackbarManager("additional_color_trackbar")
additional_color_trackbar.add_multiple(config.ADDITIONAL_COLOR_SETTING)
...
try:
    while(True):
        ...
        # --- 2. Segmentasi Warna (Masking) pada BEV Frame - TIGA WARNA DIGABUNGKAN (RGB) ---
        # TIDAK perlu konversi ke HSV lagi, langsung gunakan warped_frame (yang BGR)

        # Morphological operations (Optional, bisa diterapkan langsung ke BGR jika diinginkan)
        # Atau bisa diterapkan setelah masking pada binary_warped jika tujuannya menghilangkan noise
        # kernel = np.ones((3,3), np.uint8)
        # warped_frame_processed = cv2.morphologyEx(warped_frame, cv2.MORPH_OPEN, kernel)
        # warped_frame_processed = cv2.morphologyEx(warped_frame_processed, cv2.MORPH_CLOSE, kernel)


        # Segmentasi Warna Pertama (Misal: Putih)
        # Format trackbar sekarang: B_low, G_low, R_low, B_high, G_high, R_high
        white_rgb_value = white_trackbar.get_values_list()
        # Urutan di OpenCV adalah BGR, jadi lower_white = [B_low, G_low, R_low]
        ...
        # Segmentasi Warna Kedua (Misal: Kuning)
        ...
        # Segmentasi Warna Ketiga (Misal: Biru atau warna tambahan lainnya)
        additional_color_rgb_value = additional_color_trackbar.get_values_list()
        lower_additional = np.array([additional_color_rgb_value[0], additional_color_rgb_value[1], additional_color_rgb_value[2]])
        upper_additional = np.array([additional_color_rgb_value[3], additional_color_rgb_value[4], additional_color_rgb_value[5]])
        mask_additional = cv2.inRange(warped_frame, lower_additional, upper_additional)

        # Gabungkan ketiga masker dengan operasi Bitwise OR
        combined_mask_1_2 = cv2.bitwise_or(mask_white, mask_yellow)
        binary_warped = cv2.bitwise_or(combined_mask_1_2, mask_additional)
        ...
        cv2.imshow('Mask Additional Color', mask_additional)
