...
additional_color_trackbar.add_multiple(config.BROWN_SETTING)
...
MIN_SPEED = 3
ANGLE_THRESHOLD = 5
SPEED_REDUCTION_FACTOR = 5
MAX_STEERING_ANGLE_SIM = 30 # Baris ini dihapus atau dikomentari
...
try:
    while(True):
        ...
        additional_color_hsv_value = additional_color_trackbar.get_values_list()
        lower_additional = np.array([additional_color_hsv_value[0], additional_color_hsv_value[1], additional_color_hsv_value[2]])
        upper_additional = np.array([additional_color_hsv_value[3], additional_color_hsv_value[4], additional_color_hsv_value[5]])
        mask_additional = cv2.inRange(hsv_eq, lower_additional, upper_additional)
        ...
        if left_fit is not None and right_fit is not None:
            ...
            # --- Perubahan di sini: Hapus batasan steering angle ---
            steering_angle_command = np.clip(steering_angle, -MAX_STEERING_ANGLE_SIM, MAX_STEERING_ANGLE_SIM)
            steering_angle_command = steering_angle # Langsung gunakan steering_angle
            # --- Akhir perubahan ---

            # --- Kontrol Kecepatan Adaptif ---
            abs_steering_angle = np.abs(steering_angle_command) # Gunakan steering_angle_command yang tidak dibatasi
            ...
            # --- Menggerakkan Mobil ---
            car.setSpeed(int(current_speed))
            car.setSteering(steering_angle_command*100) # Mengirim nilai steering_angle_command yang tidak dibatasi
            print(f"Speed: {current_speed:.2f}, Steering: {steering_angle_command:.2f}") # Untuk debugging di console
