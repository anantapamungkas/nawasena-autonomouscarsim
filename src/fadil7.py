...
try:
    while(True):
        ...
        # --- 2. Segmentasi Warna (Masking) pada BEV Frame - DETEKSI JALAN (HITAM/ABU-ABU) ---
        ...
        # Di sini, kita TIDAK MENGINVERT masker.
        # binary_warped sekarang akan menunjukkan JALAN sebagai putih dan non-jalan sebagai hitam.
        # Ini adalah kebalikan dari strategi sebelumnya.
        binary_warped = mask_road # Menggunakan mask_road langsung

        # Opsional: Terapkan morphological operations untuk menghaluskan mask
        ...
        # Sekarang mencari LOKASI DENGAN PIKSEL HITAM PALING TINGGI (intensitas paling RENDAH)
        # Jika binary_warped adalah 0 untuk hitam dan 255 untuk putih,
        # maka untuk mencari konsentrasi hitam, kita perlu mencari nilai MINIMUM pada histogram,
        # atau menjumlahkan (255 - pixel_value) untuk mencari puncak.
        # Cara termudah adalah mencari MINIMUM pada histogram asli.

        # Jika binary_warped memiliki 255 untuk jalan (putih) dan 0 untuk marka (hitam)
        # Maka histogram akan memiliki puncak di mana jalan berada.
        # Kita ingin menemukan "lembah" atau "penurunan" di antara dua puncak jalan,
        # atau mencari batas terluar dari puncak jalan.

        # Mari kita asumsikan yang Anda maksud adalah: kita ingin menemukan TEPI LUAR dari AREA JALAN (yang sekarang putih).
        # Ini akan berfungsi seperti menemukan tepi marka jalan yang "normal".

        # Opsi 1: Tetap menggunakan np.argmax tapi pada histogram inverted
        # Ini akan menemukan konsentrasi pixel gelap
        # histogram_inverted = np.sum(255 - binary_warped[binary_warped.shape[0]//2:,:], axis=0)
        # leftx_base = np.argmax(histogram_inverted[:midpoint])
        # rightx_base = np.argmax(histogram_inverted[midpoint:]) + midpoint

        # Opsi 2: Mencari lembah (minimum) di antara puncak jalan
        # Ini lebih kompleks karena butuh thresholding dan filtering
        # Untuk simplicity, kita akan tetap mencari PUSAT JALUR DENGAN AREA TERGELAP (MARKAH)
        # ini berarti kita mencari min pada histogram ASLI binary_warped

        histogram = np.sum(binary_warped[binary_warped.shape[0]//2:,:], axis=0)
        midpoint = np.int32(histogram.shape[0]//2)

        # Strategi:
        # 1. Temukan puncak-puncak yang mewakili area JALAN (hitam di kamera, jadi putih di binary_warped).
        #    Ini akan menjadi dua puncak tinggi di histogram.
        # 2. Asumsikan marka jalan ada di antara atau di tepi puncak-puncak ini.

        # Ini adalah pendekatan yang lebih umum:
        # Tetap mencari puncak terang untuk sisi kiri dan kanan jalan.
        # Logika "pinggir jalan coklat" dan "marka putih/kuning" adalah yang akan kita coba ambil sebagai garis.
        # Histogram yang mencari nilai tertinggi akan menemukan jalan (putih) jika binary_warped adalah jalan.
        # Untuk menemukan MARKER, kita perlu histogram piksel NON-JALAN.

        # JIKA binary_warped adalah: Putih = Jalan, Hitam = Marka/Pinggir Jalan
        # MAKA untuk mencari marka, kita perlu menginvert binary_warped lagi untuk histogram.
        # Atau, kita harus mencari nilai MINIMUM pada histogram `binary_warped` yang asli.

        # Mari kita ambil pendekatan kedua, yang lebih intuitif:
        # Invert binary_warped sebelum menghitung histogram untuk sliding window,
        # sehingga marka dan pinggir jalan kembali menjadi putih.
        # Ini akan membuat logika sliding window tetap konsisten.

        # --- Invert binary_warped HANYA untuk perhitungan histogram dan sliding window ---
        # Ini adalah kunci untuk mempertahankan logika sliding window yang mencari PIKSEL PUTIH
        binary_warped_for_lane = cv2.bitwise_not(binary_warped)


        histogram_for_lane = np.sum(binary_warped_for_lane[binary_warped_for_lane.shape[0]//2:,:], axis=0)
        leftx_base = np.argmax(histogram_for_lane[:midpoint])
        rightx_base = np.argmax(histogram_for_lane[midpoint:]) + midpoint
        ...
        window_height = np.int32(binary_warped_for_lane.shape[0] // nwindows) # Gunakan binary_warped_for_lane

        nonzero = binary_warped_for_lane.nonzero() # Gunakan binary_warped_for_lane
        ...
        out_img = np.dstack((binary_warped_for_lane, binary_warped_for_lane, binary_warped_for_lane)) * 255 # Tampilan debug
        ...
        ploty = np.linspace(0, binary_warped_for_lane.shape[0] - 1, binary_warped_for_lane.shape[0]) # Gunakan binary_warped_for_lane
        ...
        if left_fit is not None and right_fit is not None:
            ...
            # Menggambar jalur di out_img (yang didasarkan pada binary_warped_for_lane)
            out_img[lefty, leftx] = [255, 0, 0]
            out_img[righty, rightx] = [0, 0, 255]

            # Area jalur overlay sekarang digambar di atas warped_frame asli
            lane_area_bev = np.zeros_like(warped_frame).astype(np.uint8) # Ubah np.zeros_like(out_img) menjadi warped_frame
            ...
            new_warp = np.zeros_like(warped_frame).astype(np.uint8) # Ubah np.zeros_like(result_bev)
        ...
        cv2.imshow('Mask Road (White is Road)', mask_road) # Ubah nama jendela
        cv2.imshow('Binary Mask (Inverted for Lane Detection)', binary_warped_for_lane) # Ubah nama jendela
